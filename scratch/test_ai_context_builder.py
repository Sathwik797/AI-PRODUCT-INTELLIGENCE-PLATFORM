...
def test_2_no_sqlalchemy_objects_passed():
    ...
    assert context.available_categories == ["Wearables", "Outdoor Gear"]
...
def test_5_category_context_included():
    context = AIProductContext(
        ...
        available_categories=["Office Furniture", "Home Decor", "Electronics"]
    )
    provider = GeminiProvider(config=GeminiConfig(api_key="dummy"))
    request = provider.build_multimodal_request(context)
    ...
    assert 'Available Platform Taxonomy Categories: ["Office Furniture", "Home Decor", "Electronics"]' in user_text
